...
S["endRunAt"] = new Setting("endRunAt", 500);
S.endRunAt.slider(0, 1000, 50);
...
S.hueDiffMode.selector(["setValue", "random"], true);

S["hueDiffVal"] = new Setting("hueDiffVal", 0);
S.hueDiffVal.slider(0, 100, 10);

S["satDiffMode"] = new Setting("satDiffMode", "setValue");
S.satDiffMode.selector(["setValue", "random"]);

S["satDiffVal"] = new Setting("satDiffVal", 0);
S.satDiffVal.slider(-50, 50, 5);

S["lightDiffMode"] = new Setting("lightDiffMode", "setValue");
S.lightDiffMode.selector(["setValue", "random"]);
...
function startOver() {
    ...
    // ensure the values stays between 0-100 when there are multiple layers, and sat and light are offset, but this can still fail with over 3 layers and large offset values
    if (layers > 1) {
        if (S.satDiffMode.v === "setValue"
        && S.satDiffVal.v !== 0) {
            sat = adjustedSat();
        }
        if (S.lightDiffMode.v === "setValue" 
        && S.lightDiffVal.v !== 0) {
            light = adjustedLight();
        }
    }
    ...
    for (let i = 0; i < layers; i++) { 
        ...
        else if (S.hueDiffMode.v === "setValue") {
        ...
        else if (S.satDiffMode.v === "setValue") {
        ...
        else if (S.lightDiffMode.v === "setValue") {
